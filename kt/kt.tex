...
\item 某物理量$A(\vec{p},\vec{x})$的平均值为$\langle A \rangle = \int A(\vec{p},\vec{x}) f_1(\vec{p},\vec{x};t) d^3 \vec{p} d^3 \vec{x}$，
\item 某物理量$A(\vec{p},\vec{x})$在空间中某点的值为$A(\vec{x};t) = \int A(\vec{p},\vec{x}) f_1(\vec{p},\vec{x};t) d^3 \vec{p}$。
\end{enumerate}
