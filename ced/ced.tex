...
\beq
\phi = \int \frac{d\omega}{2\pi} e^{i\omega ( t - t_0)}  d\phi\int_{k=0}^\infty \frac{k^2 dk}{(2\pi)^3} \frac{1}{k^2 - \omega^2} \int_{\theta = 0}^\pi e^{ikr\cos \theta } \sin \theta d\theta \int_{\phi = 0}^{2\pi}
\eeq
最后两个个角向积分很容易进行
\bea
...
\bea
\phi(\vec{x},t) &=\frac{1}{4\pi \epv}  \int dt'd^3 \vec{x}' \frac{1}{|\vec{x}-\vec{x}'|} \rho(\vec{x}',t') \frac{1}{2}\left[\delta (t-t_0-r) + \delta (t-t_0+r) \right]\\
...
\eeq
\par
可以验证，推迟势和超前势都各自满足有源的势函数波动方程\footnote{文中这种组合出现的原因是上一节算格林函数时，加$i \epsilon$的方法有些任意性。}，但超前势却面临严重的因果律问题（未来时间对现在世界的影响），因此，我们以后将只考虑推迟势。势对源的响应因此概括为
\bea \label{retp}
...
\beq
\int_{r=0}^\infty e^{-\frac{r^2}{2a^2}} J_0(kr) rdr = a^2 e^{-{a^2k^2}{2}}
\eeq
再带入积分就得到
\beq
