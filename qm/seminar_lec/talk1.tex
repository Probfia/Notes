...
\frametitle{\bch 证明方法2：平移算符 \ech}
\bch
由于$[H,T_{na}]=0$，设$\psi(x)$为$H$的一个本征函数，$\psi(x)$也一定是$T_a$的一个本征函数，于是有
$$
\psi(x+a) = T_a \psi(x) = \psi(x) e^{ika}
$$
$k$的取值精确到$\frac{2\pi}{a}$的整数倍，于是限制$k$在布里渊区$-\frac{\pi}{a} < k < \frac{\pi}{a}$内。
\par
当$a \to 0$，结果恢复到连续平移对称性的结果。
\par
布里渊区中的$\hbar k$称为晶体动量。超过第一布里渊区的动量需要映回第一布里渊区。直观上看，$T_{x}$和$T_{x+a}$无法区分，因此，讨论$|k| > \frac{\pi}{a}$没有意义。
\ech
\end{frame}
